...
export async function getLast24hVisitors(): Promise<number | null> {
  ...
  if (!PLAUSIBLE_SITE_ID) { 
    console.error("Plausible Site ID (PLAUSIBLE_SITE_ID) for server-side API calls is not configured.");
    return null;
  }
  ...
  try {
    ...
    console.error("Unexpected Plausible API response structure or empty results:", JSON.stringify(data, null, 2));
    return null;
...
export async function getLast7DaysVisitors(): Promise<number | null> {
  ...
  try {
    ...
    console.error("Unexpected Plausible API response structure for 7-day query:", JSON.stringify(data, null, 2));
    return null;
...
export async function getLast30DaysVisitors(): Promise<number | null> {
  ...
  try {
    ...
    console.error("Unexpected Plausible API response structure for 30-day query:", JSON.stringify(data, null, 2));
    return null;
